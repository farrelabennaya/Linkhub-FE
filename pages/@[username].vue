...
const getThemeClasses = (theme: string) => {
  const themes = {
    ...
    neon: {
      ...
      avatar:
        "ring-4 ring-gradient-to-r from-purple-500 to-pink-500 shadow-2xl shadow-purple-500/50",
    },
  };
...
<template>
  <main>
    ...
    <div
      v-if="loading"
      class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900"
    >
      <div class="text-center">
        <div
          class="animate-spin rounded-full h-16 w-16 border-b-4 border-cyan-400 mx-auto mb-4"
        ></div>
    ...
    >
      ...
      >
        <div
          class="absolute top-20 left-20 w-96 h-96 bg-purple-500/30 rounded-full filter blur-[128px] animate-pulse"
        ></div>
        <div
          class="absolute bottom-20 right-20 w-96 h-96 bg-pink-500/30 rounded-full filter blur-[128px] animate-pulse"
          style="animation-delay: 1s"
        ></div>
      </div>

      <div
        v-if="data.profile.theme === 'dark'"
        class="absolute inset-0 overflow-hidden pointer-events-none"
      >
        <div
          class="absolute top-0 left-1/4 w-96 h-96 bg-cyan-500/20 rounded-full filter blur-[128px]"
        ></div>
        <div
          class="absolute bottom-0 right-1/4 w-96 h-96 bg-blue-500/20 rounded-full filter blur-[128px]"
        ></div>
      </div>
      ...
      <div class="relative z-10 w-full max-w-2xl mx-auto">
        ...
        <div class="flex flex-col items-center mb-8 animate-fadeIn">
          <div v-if="data.profile.avatar_url" class="relative group mb-6">
            ...
            <div
              v-if="data.profile.theme === 'neon'"
              class="absolute inset-0 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 animate-spin-slow opacity-50 blur-md -z-10"
            ></div>
          </div>
        ...
        <div class="w-full space-y-4 px-4">
          ...
          >
            ...
            <div
              v-if="data.profile.theme === 'minimal'"
              class="absolute inset-0 border-2 border-transparent group-hover:border-gray-900 rounded-2xl transition-colors duration-300"
            ></div>
          </a>
        ...
        >
          <p :class="['text-sm', getThemeClasses(data.profile.theme).subtext]">
            Powered by
            <a
              href="https://linkhub-fe.vercel.app"
              target="_blank"
              rel="noopener noreferrer"
              class="inline-flex items-center font-semibold text-cyan-400 hover:text-cyan-300 underline decoration-dotted underline-offset-4 transition-colors"
              aria-label="Buka HulaHub di tab baru"
            >
              HulaHub
            </a>
          </p>
